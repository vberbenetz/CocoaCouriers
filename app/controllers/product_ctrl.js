...
productCtrl.prototype = {
    ...
    list: function (dbConnPool, callback) {
        ...
        var query = {
            statement: 'SELECT p.*, m.name as m_name, m.description as m_description, m.origin as m_origin, m.website as m_website FROM Product p INNER JOIN Manufacturer m ON p.manufacturer_id=m.id ORDER BY p.displayPriority DESC Limit 40',
            params: []
        };
    ...
    listByFilter: function(conditions, dbConnPool, callback) {
        ...
        // No params added, use default query
            ...
            query.statement = 'SELECT p.*, m.name as m_name, m.description as m_description, m.origin as m_origin, m.website as m_website FROM Product p INNER JOIN Manufacturer m ON p.manufacturer_id=m.id ORDER BY p.displayPriority DESC Limit 40';
            dbUtils.query(dbConnPool, query, function(err, rows) {
                if (err) {
                    return callback(err, null);
                }
